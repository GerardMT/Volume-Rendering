...
void Line::initialieGL()
{
    if (instances_ == 0) {
        ...
        bool res = program_->addShaderFromSourceFile(QOpenGLShader::Vertex, "../shader/line.vert");
        if (!res){
            cout << program_->log().toUtf8().constData() << endl;
        }
        res = program_->addShaderFromSourceFile(QOpenGLShader::Fragment, "../shader/line.frag");
        if (!res) {
            cout << program_->log().toUtf8().constData() << endl;
        }
        program_->link();
