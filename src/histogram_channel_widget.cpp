...
void HistogramChannelWidget::initializeGL()
{
    ...
    // Histogram
    bool res = program_histogram_.addShaderFromSourceFile(QOpenGLShader::Vertex, "../../res/shader/histogram.vert");
    if (!res){
        cout << program_histogram_.log().toUtf8().constData() << endl;
    }
    res = program_histogram_.addShaderFromSourceFile(QOpenGLShader::Fragment, "../../res/shader/histogram.frag");
    if (!res) {
        cout << program_histogram_.log().toUtf8().constData() << endl;
    }
    ...
    // Points
    res = program_point_.addShaderFromSourceFile(QOpenGLShader::Vertex, "../../res/shader/histogram_point.vert");
    if (!res){
        cout << program_point_.log().toUtf8().constData() << endl;
    }
    res = program_point_.addShaderFromSourceFile(QOpenGLShader::Fragment, "../../res/shader/histogram_point.frag");
    if (!res) {
        cout << program_point_.log().toUtf8().constData() << endl;
    }
    program_point_.link();
    ...
    // Lines
    res = program_line_.addShaderFromSourceFile(QOpenGLShader::Vertex, "../../res/shader/histogram_line.vert");
    if (!res){
        cout << program_line_.log().toUtf8().constData() << endl;
    }
    res = program_line_.addShaderFromSourceFile(QOpenGLShader::Fragment, "../../res/shader/histogram_line.frag");
    if (!res) {
        cout << program_line_.log().toUtf8().constData() << endl;
    }
    program_line_.link();
